...
import { getV2ReadFileTool } from "./../../services/harness/tools/read";
import { Tool as TodoTool } from "./../../services/harness/tools/todo";
import { Tool as AskUserQuestionTool } from "./../../services/harness/tools/askUserQuestion";
import { getTaskTool } from "./../../services/harness/tools/task";
import { getGlobTool } from "./../../services/harness/tools/search/glob";
import { ChatApplyType, useChatApplyStore } from '../chatApply';
import { getEditTool, getWriteTool } from "./../../services/harness/tools/write";
export function getToolsEN(options: {
  ...
  const planModeState =
    useChatStore.getState().currentSession()?.data?.planModeState || 'off';
  ...
  if (codeMakerVersion) {
    if (isVSCode) {
      ...
      if (codeMakerVersion.includes('-')) {
        ...
        enableGrep =
          !!pluginVersion && versionCompare('2.4.4', pluginVersion) >= 0;
      }
    }
  }

  let enablePlanMode =
    useChatStore.getState().currentSession()?.data?.enablePlanMode || false;
      ...
      function: {
        ...
        parameters: {
          properties: {
            ...
            is_create_file: {
              description:
                'Whether the target file is a new file or an existing one. Defaults to false. If true, the target file is a new file.',
              type: 'boolean',
            },
          },
...
4. Special operations:
  ...
  * To delete code: Use empty REPLACE section`,
  ...
  if (enablePlanMode) {
    if (
      planModeState === 'off' ||
      planModeState === 'pending_approval' ||
      !planModeState ||
      ...
      function: {
        ...
        parameters: {
          ...
          properties: {
            server_name: {
              type: 'string',
              description: '提供该资源的MCP服务器名称',
            },
            uri: {
              type: 'string',
              description: '用于标识特定资源的URI',
            },
          },
