...
import { CODEBASE_SESSION_CREATED_EVENT, dispatchTourEvent } from '../components/FeatureTour';
import {
...
import { Docset, Docsets, DocsetType } from '../services/docsets';
import { DEFAULT_MASKS, IS_PROGRAMMING_MODE, useMaskStore } from './mask';
import {
  subagentCoordinator,
  useSubagentStore,
  TaskResult,
...
import { cloneDeep, findLastIndex, get as lodashGet, isArray } from 'lodash';
import {
  CHAT_MAX_TOKENS,
  CHAT_MIN_TOKENS,
...
import { PluginAction, PluginAppRunner, PluginAppRunnerParams, PluginRecieveData } from '../services/plugin';
...
import { truncateSessionTopic, versionCompare } from '../utils/common';
...
import { CACHE_TIER_BREAK } from './workspace/constructRemixPrompt';
import { usePromptApp } from './promp-app';
import { CFG_PROMPT, CLASS_PROMPT, ER_PROMPT, MINDMAP_PROMPT, SEQUENCE_PROMPT } from '../utils/prompt';
import { generateTraceId } from '../utils/trace';
import { hub as webToolsHub, logger as webToolsLogger } from '@dep305/codemaker-web-tools';
...
import { processMakePlanDenied, report as planReport, Tool as PlanTool } from '../store/workspace/tools/plan';
...
import { BAI_CHUAN, ChatModel, ParseImgType } from '../services/chatModel';
...
import { isAbortError, REQUEST_TIMEOUT_NAME } from '../utils/abort';
import {
...
import { ChatStreamErrorMgr } from '../utils/error';
import { PromptLinkMgr } from './workspace/pomptLinkMgr';
import { getBuiltinSkillContent } from './skills/builtinSkills';
import {
  AgentTaskDirective,
...
import { createToolCallStreamParser } from '../utils/chunk';
...
interface ChatStore {
  ...
  onNewSession: (
    message?: ChatMessage[],
    options?: { chat_source?: string; raw_user?: string },
  ) => Promise<void>;
  // 更新当前会话
  ...
  updateApplyingInfo: (info: ChatStore['applyingInfo'] | null) => void;
  // 压缩相关方法
 ...
 * 获取当前的自动执行权限配置
 */
function getCurrentAutoPermissions(): AutoExecutePermissions {
  const config = useChatConfig.getState();
  return {
  ...
  persist(
    (set, get) => ({
      ...
      revalidateChatSessions: async (
        ...
        if (
          ...
          return;
        // 当前会话正在同步写入服务端时，跳过校验，避免用服务端旧数据覆盖本地
        const _currentSessionId = get().currentSessionId;
        if (_currentSessionId && pendingSyncCounts.has(_currentSessionId))
          return;
        const filterData = data.filter(
          (item) => item.chat_type === chatType && !item.is_favorite,
        );
        let currentSessionId = _currentSessionId;
        ...
        if (!filterData.length) {
          ...
          if (chatType === 'codebase' && workspaceInfo.repoName) {
            ...
            if (currentSessionId) {
              targetSession = filterData.find(
                (item) =>
                  ...
                  (!item.chat_repo ||
                    item.chat_repo === workspaceInfo.repoName),
              );

              // 当前会话可能刚创建尚未出现在服务端列表中，从本地 pending 状态查找
              if (
                !targetSession &&
                pendingSyncSessionIds.has(currentSessionId)
              ) {
                const localSession = currentSessions.get(currentSessionId);
                if (
                  localSession?.chat_type === chatType &&
                  (!localSession.chat_repo ||
                    localSession.chat_repo === workspaceInfo.repoName)
                ) {
            ...
            if (!targetSession) {
              targetSession = filterData.find(
                (item) =>
                  !item.chat_repo || item.chat_repo === workspaceInfo.repoName,
              );
            }
            if (targetSession) {
          ...
          if (
            currentSessionId &&
            !nextSessions.has(currentSessionId) &&
            pendingSyncSessionIds.has(currentSessionId)
          ) {
            const localSession = currentSessions.get(currentSessionId);
      ...
      onNewSession: async (messages, options) => {
        ...
        const sessionParams: {
          ...
          data: {
            ...
            enablePlanMode:
              !!get()?.currentSession()?.data?.enablePlanMode || false,
        ...
        if (chatType === 'codebase') {
          ...
          sessionParams.data.codebase_chat_mode =
            chatType === 'codebase' ? get().codebaseChatMode : undefined;
          set({
            codebaseChatMode:
              chatType === 'codebase' ? get().codebaseChatMode : undefined,
            activeChangeId: undefined,
        ...
        if (chatType === 'codebase') {
          try {
            useHookStore.getState().emit('session.start', {
      ...
      forkFavoriteSession: async (sourceSessionId: string) => {
        ...
        if (!sourceSession || !sourceSession.data) return;

        const chatType = sourceSession.chat_type || get().chatType;
        ...
        const sessionParams: {
          ...
          data: {
            ...
            codebase_chat_mode: sourceSession.data.codebase_chat_mode,
          },
        };

        if (chatType === 'codebase' && sourceSession.chat_repo) {
          sessionParams.chat_repo = sourceSession.chat_repo;
        }

        // 切换到对应的 chatType
        if (get().chatType !== chatType) {
          set({ chatType });
        }
      ...
      loadSessionData: async (
        ...
        const resumeAttaches = (cloudAttachs: IMultiAttachment | Docsets) => {
          ...
          if (chatType !== 'codebase') {
            ...
            const filterAttachs = (
              ...
              (i) =>
                (i.attachType === AttachType.File &&
                  (i as FileItem)?.isCurrent) ||
          ...
          if (attachs?.attachType === AttachType.MultiAttachment) {
            ...
            const cloudSource = (
              ...
              [AttachType.CodeBase, AttachType.Docset].includes(i?.attachType),
            );
            const newSource = localSource?.filter(
              (local) =>
                (local.attachType === AttachType.File &&
                  (local as FileItem)?.isCurrent) ||
                local.attachType === AttachType.ImageUrl ||
                cloudSource.some(
                  (cloud) =>
                    local.attachType === AttachType.File &&
                    local.attachType === AttachType.File &&
                    (local as FileItem)?.path !== (cloud as FileItem)?.path,
                ) ||
                cloudSource.some(
                  (cloud) =>
                    local.attachType === AttachType.CodeBase &&
                    local.attachType === AttachType.CodeBase &&
                    (local as CodeBase)?.collection !==
                    (cloud as CodeBase)?.collection,
                ) ||
                cloudSource.some(
                  (cloud) =>
                    local.attachType === AttachType.Docset &&
                    local.attachType === AttachType.Docset &&
                    (local as Docset)?._id !== (cloud as Docset)?._id,
                ),
            );
            update({
              attachType: AttachType.MultiAttachment,
        ...
        if (isPending) {
          try {
            ...
            console.warn(
              `[Debug] loadSessionData failed for session ${id}:`,
              error?.response?.status || error?.message,
            );
            if (error?.response?.status === 404 && isPending) {
              console.log(
                `[Debug] Session ${id} not found, creating new session`,
              );
              get().onNewSession();
            } else {
      ...
      currentSession: () => {
        ...
        if (
          session &&
          (!session.data || !Array.isArray(session.data.messages))
        ) {
          // 修复 session 数据结构
      ...
      removeSession: async (id: string) => {
        ...
        if (chatType === 'codebase') {
          try {
            useHookStore.getState().emit('session.end', {
              sessionId: id,
            });
          } catch (e) {
            void e;
      ...
      async selectSession(id: string, callback?: () => void) {
        try {
          ...
          // 切换会话时先清空，避免上一个会话的脏值残留到新会话
          if (get().chatType === 'codebase') {
            set({
              activeChangeId: undefined,
              activeFeatureId: undefined,
            });
      ...
      clearSession: async () => {
        ...
        const newSession = {
          ...
          data: {
            ...
            messages: [],
            consumedTokens:
              session.data?.consumedTokens || createInitialConsumedTokens(),
          },
        };
        try {
      ...
      updateConsumedTokens: (options: {
        ...
        if (!curSession.data) {
          console.warn(
            '[updateConsumedTokens] Session data not found, skipping token update',
          );
          return;
        }

        // 确保 consumedTokens 已初始化
        ...
        try {
          ...
          if (result.compressionUpdate && curSession.data.compression) {
            curSession.data.compression.pendingSavedTokens =
              result.compressionUpdate.pendingSavedTokens;
            curSession.data.compression.messagesCountAtCompression =
              result.compressionUpdate.messagesCountAtCompression;
          }

          // 将当前累计 token 快照写入最后一条消息，用于计算每轮增量
          // 与 ChatConsumeTokenPanel 的 totalTokens 计算保持一致
          ...
          if (ct) {
            ...
            try {
              otel.setSessionTokenDistributionOnRound(ct, {
                chatType: get().chatType,
                round: useChatStreamStore.getState().conversationRound,
              });
            } catch {
              /* OTEL 失败不影响 token 更新 */
            }
          ...
          console.error(
            '❌ %cToken Update Failed:',
            'color: #dc2626; font-weight: bold; background: #fee2e2; padding: 2px 6px; border-radius: 4px;',
            {
              '🆔 Session ID':
                curSession._id?.substring(0, 8) + '...' || 'Unknown',
              '🤖 Model': model || 'Unknown',
              '📊 Input': tokenIncrement,
              '⚠️ Error': error,
            },
          );
          throw error; // 重新抛出错误，保持原有的错误处理行为
        }
      },
      ...
      async syncHistory() {
        const session = get().currentSession();
        // 目前发现用户在网络异常或资源加载异常时，有时会触发上下文被清空
        // 如果没有上下文，则不进行同步,一般清空会话、删除QA、删除会话都不走这里
        ...
        pendingSyncCounts.set(
          sessionId,
          (pendingSyncCounts.get(sessionId) || 0) + 1,
        );
        try {
          await updateSession(latestData);
        } catch (error) {
      ...
      analyzeContext: async (sessionId: string) => {
        ...
        //   'claude-sonnet-4-5-20250929': 16800,
        // } as Record<string, number>;

        const pendingSavedTokens =
          session.data.compression?.pendingSavedTokens || 0;
        ...
        return {
          ...
          thresholds: {
            ...
            percentLeft: Math.max(
              0,
              Math.round((1 - analysis.percentageUsed) * 100),
            ),
          },
        };
      },
      ...
      triggerCompression: async (sessionId) => {
        const round = useChatStreamStore.getState().conversationRound;
        const compressionSpan = otel.startCompressionSpan(sessionId, round);
        try {
          ...
          if (currentStatus === SessionStatus.COMPRESSING) {
            ...
            otel.stopCompressionSpan(compressionSpan, {
              success: false,
          ...
          if (!session?.data?.messages) {
            otel.stopCompressionSpan(compressionSpan, {
              success: false,
              error: 'No session or messages found',
            });
            return false;
          }

          const readyToCompress = session.data.messages.filter(
            (msg) => !msg.isCompressed,
          );
          ...
          const compressionResult =
            await compressionService.performCompression(compressionContext);
          ...
          if (
            ...
            if (compressionResult.uncompressedMessages) {
              compressedMessages = readyToCompress.filter(
                (msg) =>
                  compressionResult.uncompressedMessages?.findIndex(
                    (uncompressedMsg) =>
                      `${uncompressedMsg.id}-${uncompressedMsg.role}` ===
                      `${msg.id}-${msg.role}`,
                  ) === -1,
              );
            }

            get().markMessagesAsCompressed(sessionId, compressedMessages);
            ...
            get().updateCurrentSession((session) => {
              if (session.data) {
                ...
                session.data.compression.totalTokensSaved +=
                  newHistory.tokensSaved;
                session.data.compression.totalCompressionsCount += 1;
                session.data.compression.pendingSavedTokens =
                  newHistory.tokensSaved;
                session.data.compression.messagesCountAtCompression =
          ...
          otel.stopCompressionSpan(compressionSpan, {
            success: false,
            error: 'Compression returned no result',
          });
          return false;
        } catch (error) {
        ...
        get().updateCurrentSession((session) => {
          if (session.data?.messages) {
            ...
            for (const msg of compressedMessages) {
              const index = session.data.messages.findIndex(
                (m) => `${m.id}-${m.role}` === `${msg.id}-${msg.role}`,
              );
              if (index !== -1) {
      ...
      inferExecutionContext(
        ...
        if (hasTaskTools) {
          // 如果有task工具，创建subagent上下文
          return createSubagentContext(messageId, sessionId);
        } else {
          // 否则创建主agent上下文
          const permissions = getCurrentAutoPermissions();
          return createMainAgentContext(messageId, sessionId, permissions);
        }
      },
    ...
    }),
    {
      ...
      storage: createJSONStorage(() => {
        ...
        // 并行会话使用 sessionStorage（临时存储，关闭即销毁）
        return mode === 'panel' ? sessionStorage : localStorage;
      }),
      partialize: (state) => ({
...
export type ChatStreamState = {
  isStreaming: boolean;
  isTurnCompleteHookFired: boolean;
  isSearching: boolean;
  ...
  submitSpan?: otel.SafeSpan;
  conversationRound?: otel.ConversationRoundState;
  runnerTask?: {
    id?: string;
  ...
  startSubmitSpan: (isUserInput: boolean) => void;
  endSubmitSpan: () => void;
...
type ChatStreamActions = {
  ...
  onUserResubmit: (hasForce?: boolean) => void;
  // // 更新 toolCall 结果
   ...
   *
   * @param params - task 工具结果参数
   * @param params.toolCallId - 工具调用 ID
   * @param params.taskResult - subagent 执行结果
   * @param params.sessionId - 目标会话 ID (通常是主会话 ID)
...
export const useChatStreamStore = create(
  immer<ChatStreamState & ChatStreamActions>((set, get) => ({
    ...
    updateToolCallResults(
      result: {
        [propName: string]: { path: string; content: string; isError?: boolean };
      },
      extra?: any,
      executionContext?: ExecutionContext,
      options?: { skipSync?: boolean; skipAutoExecute?: boolean },
    ) {
      // Fire tool.afterExecute hook (fire-and-forget)
      try {
        const toolIds = Object.keys(result);
        for (const toolId of toolIds) {
          const session = useChatStore.getState().currentSession();
          const toolCall = session?.data?.messages?.[
            (session?.data?.messages?.length || 1) - 1
          ]?.tool_calls?.find((tc) => tc.id === toolId);
          let toolArgs: Record<string, unknown> = {};
          try {
            toolArgs = JSON.parse(toolCall?.function?.arguments || '{}');
          } catch { /* ignore */ }
          useHookStore.getState().emit('tool.afterExecute', {
            sessionId: useChatStore.getState().currentSession()?._id || '',
          ...
          if (
            toolCall &&
            ['edit_file', 'replace_in_file', 'write_to_file', 'create_file'].includes(
            ...
            useHookStore.getState().emit('file.afterEdit', {
              ...
              toolCallId: toolId,
              toolName: toolCall.function.name,
              path: toolArgs.target_file || toolArgs.path || '',
              success: !result[toolId]?.isError,
            });
          }

          if (toolCall && ['run_terminal_cmd'].includes(toolCall.function.name)) {
            let toolArgs: any = {};
            try {
              toolArgs = JSON.parse(toolCall.function.arguments || '{}');
            } catch { /* empty */ }
      ...
      chatStoreState.updateCurrentSession((session) => {
        if (session && session.data) {
          ...
          console.log('[Debug][updateToolCallResults] 检查是否所有工具完成:', {
            toolCallsCount: lastMessage.tool_calls?.length || 0,
            toolResultCount: Object.keys(lastMessage.tool_result || {}).length,
            hasActiveTaskTools,
            allTasksHaveResults,
            trackerSessionComplete,
          });

          // ✅ 使用 TaskCompletionTracker 进行统一判断
          if (taskToolCalls.length > 0) {
            // 有 task 工具，使用 Tracker 判断完成
            const tracker = useTaskCompletionStore.getState();
            const isComplete = tracker.isSessionComplete(currentSessionId || '');

            if (!isComplete) {
              console.log(
                '[Debug][updateToolCallResults] Task 工具尚未完成，等待中...',
                {
                  pendingCount: tracker.getPendingTaskCount(currentSessionId || ''),
                }
              );
              return;
            }

            console.log('[Debug][updateToolCallResults] 所有 task 工具完成');
          }
          ...
          if (
            ...
            console.log('[Debug][updateToolCallResults] 所有工具完成，准备调用 handleAutoExecute ');
            isProcessing = false;
            ...
            if (!options?.skipAutoExecute) {
              ...
              if (import.meta.env.DEV) {
                console.log(
                  '[Debug][updateToolCallResults] skipAutoExecute=true, deferring to event-driven completion',
                );
              }
            }
          }
    ...
    findCorrectToolMessageIndex(messages: ChatMessage[], toolCallId: string): number {
      // 找到包含该 tool_call_id 的 Assistant 消息
    ...
    async updateTaskToolResult(params) {
      ...
      const release = await sessionUpdateLock.acquire(sessionId, 60000); // 60秒超时
      ...
      try {
        ...
        // 🔍 完整性验证
        const freshSession = useChatStore.getState().sessions.get(sessionId);
        if (freshSession?.data?.messages) {
          const integrityReport = validateToolResultIntegrity(
            freshSession.data.messages,
            sessionId
          );
        ...
        if (import.meta.env.DEV) {
          console.log(
            `[updateTaskToolResult] 🔓 Lock released for session ${sessionId}`
          );
        }
      }
    },
    ...
    onUserResubmit(hasForce = false) {
      const { isStreaming, isSearching, isMCPProcessing } = get();
      if (!hasForce && (isStreaming || isSearching || isMCPProcessing)) {
        return;
      }
      const chatStoreState = useChatStore.getState();
    ...
    onUserSubmit: async (
      ...
      useChatStreamStore.setState({ isTurnCompleteHookFired: false });
      const isUserInput = content.trim().length > 0;
      if (chatType === 'codebase' && !options?.isMCPToolResponse && isUserInput) {
        try {
          const hookResult = await useHookStore
            .getState()
            .emitBlockable('message.userSubmit', {
              sessionId: chatStoreState.currentSession()?._id || '',
              content,
            });
      ...
      get().startSubmitSpan(isUserInput);
      ...
      if (mcpRunnerState.runner && mcpRunnerState.resultText) {
        userInputContent =
          mcpRunnerState.resultText + '\n\n' + userInputContent;
      }
      const skillRunnerState = useSkillPromptApp.getState();
      if (skillRunnerState.activeSkills.size > 0 && skillRunnerState.resultText) {
        userInputContent =
          skillRunnerState.resultText + '\n\n' + userInputContent;
      }
      ...
      if (ntesTraceId) {
        set((state) => {
          if (state.conversationRound) {
            state.conversationRound.ntesTraceId = ntesTraceId;
            state.conversationRound.rootSpan?.setAttribute(
              'ntes.trace_id',
              ntesTraceId,
            );
            state.conversationRound.submitSpan?.setAttribute(
              'ntes.trace_id',
              ntesTraceId,
            );
          }
      ...
      const submitOptions = ntesTraceId
        ? {
          ...options,
          ntesTraceId,
        }
      ...
      const hasMentionKnowledgeBases = !!(
        attachs as IMultiAttachment
      )?.dataSource?.some((i) => i?.attachType === AttachType.Docset);
      const currentSessionId = useChatStore.getState().currentSessionId;
      ...
      if (attachs) {
        if (
          ...
          if (
            chatModels[chatConfig.model]?.parseImgType !== ParseImgType.NONE &&
            imgUrls.length
          ) {
      ...
      // 支持多skill: 从activeSkills中获取所有skills
      const activeSkills = Array.from(skillRunnerState.activeSkills.values());
      if (activeSkills.length > 0) {
        // 如果有多个skills，使用第一个作为主skill展示（兼容单skill模式）
        ...
        import('../services/skillUsage')
          .then(({ reportSkillInvoke }) => {
            import('./skills').then(({ getSkillDescription }) => {
              activeSkills.forEach(skill => {
                const description = getSkillDescription(skill.skillName);
                reportSkillInvoke(skill.skillName, {
      ...
      if (chatType === 'codebase' && effectiveRules.length) {
        // 去重规则
        effectiveRules = Array.from(
          new Map(effectiveRules.map((rule) => [rule.filePath, rule])).values()
        );
      ...
      if (toolResponse) {
        console.log(`[DEBUG] toolResponse:`, toolResponse);
        ...
        if (session.data.messages.length) {
          ...
          if (
            ...
            lastMessage.tool_calls.forEach((tool) => {
              ...
              if (
                ['edit_file', 'replace_in_file', 'write', 'edit'].includes(tool.function.name)
              ) {
        ...
        chatStoreState.updateCurrentSession((session) => {
          session.chat_repo = session.chat_repo || workspaceInfo.repoName;
          console.log('debug', rejectResponse, session?.data?.messages);
            ...
            if (rejectResponseIds.length) {
              rejectResponseIds.forEach((toolId, index) => {
                ...
                session.data?.messages.push({
                  ...
                  tool_call_id: toolId,
                  content:
                    tool?.function.name === PlanTool.function.name
        ...
        userMessage.attachs = await getParsedAttachs(
          userMessage.attachs as MultipleAttach[],
        );
          ...
          [],
          {
            ...
            onError(error) {
              ...
              if (
                ...
                specialErrorPatterns[1].condition(error.message)
              ) {
                get().retryChatStream(
                  id,
                  error.name === REQUEST_TIMEOUT_NAME
                    ? ERetryType.Timeout
                    : ERetryType.AccessLimit,
                );
                return;
              }
      ...
      if (attachs && attachs.attachType === AttachType.Docset) {
        ...
        chatStoreState.updateCurrentSession((session) => {
          session.data = session.data || {
            messages: [],
            consumedTokens: createInitialConsumedTokens(),
            attaches: { docsets: [], attachType: AttachType.Docset },
          };
          session.data.attaches = session.data.attaches || {
            docsets: [],
            attachType: AttachType.Docset,
          };
          session.data.attaches = {
            attachType: AttachType.Docset,
            docsets: filterDocsetsFn(docsets) as Docset[],
          };
          return session;
        });
        if (docsets.length > 1) {
            ...
            onMessage(content, done, jsonData) {
              ...
              if (done) {
                ...
                chatStoreState.updateConsumedTokens({
                  ...
                  completionTokens:
                    jsonData?.metadata?.usage?.completion_tokens || 0,
                  cacheCreationInputTokens:
                    jsonData?.metadata?.usage?.cache_creation_input_tokens || 0,
                  cacheReadInputTokens:
                    jsonData?.metadata?.usage?.cache_read_input_tokens || 0,
                });
                get().endConversationRound();
                chatStoreState.syncHistory();
              } else {
        ...
        attachs &&
        attachs.attachType === AttachType.KnowledgeAugmentation
      ) {
        // 走 BM 知识增强的逻辑
        // 重置
        get().reset();
      ...
      if (attachs && attachs.attachType === AttachType.NetworkModel) {
        // 走联网模型逻辑
        get().reset();
        set(() => ({ isStreaming: true }));
        const { model } = attachs as NetworkModel;
        ...
        requestNetworkChatStream(options.event, data, chatRequestUrl, {
          ...
          onError(error) {
            ...
            // 检测限额以及访问服务时http错误
            if (
              ChatStreamErrorMgr.instance.checkStreamExceptionFromService({
                messageId: id,
                error,
                sessionId: session._id,
        ...
        return;
      } else if (chatStoreState.chatType === 'codebase') {
        // 走 CodeBase Chat 逻辑
        get().reset();
        set(() => ({ isStreaming: true }));
        ...
        const compressionSummaryCountAtRequest = (
          session.data?.messages || []
        ).filter((msg) => msg.isCompressionSummary).length;

        const unCompressedMessages = (session.data?.messages || []).filter(
          (msg, index, self) =>
            !msg.isCompressed ||
        ...
        const useCompression =
          compressionEnabled && currentCompressStatus !== SessionStatus.FAILED;
        ...
        if (cacheEnable) {
          // 多 cache breakpoint：按 tier 分隔符切分为多个 content block
        ...
        configureThinkingSignature(chatModels[chatConfig.model], data);
        ...
        if (
          [ChatModel.Gemini25, ChatModel.Gemini3Pro].includes(chatConfig.model)
        ) {
        ...
        data.max_tokens = Math.max(
          data.max_tokens || 0,
          DEFAULT_MAX_TOKENS,
          chatModels[chatConfig.model]?.tokenInfo?.maxOutputTokens || 10240,
        );
        // TEST: 调试用
        ...
        let inDiff = false;
        const parseWriteEditToolCall = createToolCallStreamParser();

        const codebaseChatMode = useChatStore.getState().codebaseChatMode;
        if (
          session.data?.enablePlanMode &&
        ...
        if (
          ...
          [ChatModel.Glm47, ChatModel.Glm5, ChatModel.Glm5Turbo, ChatModel.Glm51].includes(data.model as ChatModel)
        ) {
          data.temperature = 2;
          data.top_p = 0.95;
        }
        ...
        // 模型探测：偷偷发 2 个请求对比带策略 vs 纯净的模型响应
        void probeModelIdentity(data, {
          model: data.model || '',
          ntesTraceId: options?.ntesTraceId,
          chatRequestUrl,
        });

        // await chatStoreState.triggerCompression(sessionId);
        requestCodebaseChatStream(
          ...
          chatRequestUrl,
          {
            onMessage(
              ...
              if (done) {
                ...
                const compressionSummaryCountNow = (
                  currentSession?.data?.messages || []
                ).filter((msg) => msg.isCompressionSummary).length;
                const hasCompressionOccurredDuringRequest =
                  compressionSummaryCountNow !==
                ...
                console.log(
                  `promptTokens: ${promptTokens}, completionTokens: ${completionTokens}, cacheCreationInputTokens: ${cacheCreationInputTokens}, cacheReadInputTokens: ${cacheReadInputTokens}`,
                );
                if (attachs) {
                  const curAttachs = useChatAttach.getState()
                    .attachs as IMultiAttachment;
                  const dataSource = (curAttachs?.dataSource || []).filter(
                    (i) =>
                      [AttachType.CodeBase, AttachType.Docset].includes(
                        i.attachType,
                      ),
                  );
                ...
                let isProcessing =
                  toolCalls.length && !allResponsed ? true : false;
                ...
                set((state) => {
                  ...
                  state.isApplying = toolCalls.some((toolCall) =>
                    ['edit_file', 'reapply', 'replace_in_file', 'edit', 'write'].includes(
                      toolCall.function.name,
                    ),
                  );
                ...
                if (
                  !streamStore.isTurnCompleteHookFired &&
                  toolCalls.length === 0
                ) {
                  useChatStreamStore.setState({
                    isTurnCompleteHookFired: true,
                  });
                  try {
                    useHookStore.getState().emit('message.complete', {
                      sessionId: chatStoreState.currentSession()?._id || '',
                    });
                  } catch (e) {
                    void e;
                ...
                chatStoreState.updateCurrentSession((session) => {
                  ...
                  if (session.data?.messages) {
                    ...
                    if (newTruncateStart >= 0) {
                      session.data.messages[newTruncateStart].truncateStart =
                        true;
                    }
                  }

                  // 过滤多余的 ask_user_question 工具调用
                  const filteredToolCalls = onMessageToolCallResponse(
                    session,
                    content,
                    done,
                    toolCalls,
                  ...
                  session.data?.messages.push({
                    ...
                    id,
                    content,
                    tool_calls: filteredToolCalls || [], // 使用过滤后的 toolCalls
                  ...
                  chatStoreState.updateConsumedTokens({
                    ...
                    // systemToolTokens: estimateSystemToolTokens(data.tools),
                ...
                userReporter.report({
                  ...
                  extends: {
                    is_model_cache_enable:
                      !!chatModels[chatConfig.model]?.hasTokenCache,
                    model: chatConfig.model,
                    ...
                    compression_enabled:
                      compressConfig.enable && compressConfig.visible,
                  },
                });
                if (toolCalls.length) {
                  if (!allResponsed) {
                      ...
                      taskToolCalls.forEach((taskTool) => {
                        ...
                        (async () => {
                          try {
                            ...
                            // 获取当前的 conversationRound（用于 OTEL 追踪）
                            const round = get().conversationRound;
                            ...
                            // 注意: 错误场景只更新 tool_result，不插入 tool message
                            try {
                              get().updateToolCallResults({
                                [taskTool.id]: {
                                  path: '',
                              ...
                              // ⚠️ 如果连 updateToolCallResults 都失败了，至少记录日志
                              console.error(
                    ...
                    const toolsToProcess = nonTaskToolCalls;
                      ...
                      if (!response[tool.id]) {
                        ...
                        if (
                          ...
                          if (autoApprove) {
                            ...
                            otel.saveToolContext(
                              tool.id,
                              tool.function.arguments,
                              get().conversationRound,
                            );
                            (async () => {
                              try {
                                  ...
                                  get().updateToolCallResults({
                                    [tool.id]: {
                                      ...
                                      content: JSON.stringify({
                                        error: true,
                                        message: `Hook denied: ${hookResult.reason || 'blocked by hook'}`,
                                      }),
                                      isError: true,
                                    },
                                  });
                            ...
                            const capturedArguments = tool.function.arguments;
                            const capturedParams = { ...params };
                            const capturedConversationRound = get().conversationRound;
                          ...
                          console.log(
                            '[Debug][toocall] retrieve_code params:',
                            tool_params,
                          );
                            ...
                            try {
                              ...
                              const hookData = {
                                ...
                                args: {
                                  ...
                                  collection:
                                    parseAtMentionedCodeBaseByAttach(),
                                },
                              };
                              const hookResult = await useHookStore
                                .getState()
                          ...
                          if (
                            ...
                            ['g83_us_package_script'].includes(
                              workspaceInfo.repoCodeTable,
                            )
                          ) {
                          ...
                          if (isImageFileByPath(tool_params.path)) {
                            set(() => ({
                              loadingMessage: '解析图片中...',
                            }));
                          }
                          console.log(
                            '[Debug][toocall] read_file params:',
                            tool_params,
                          );
                          (async () => {
                            try {
                          ...
                          console.log(
                            '[Debug][toocall]  terminal cmd params:',
                            tool_params,
                          );
                          if (
                            useChatConfig.getState().autoExecute &&
                            isCommandSafe(
                              useConfigStore.getState().config
                            ...
                            (async () => {
                              try {
                                ...
                                const termResult = await useHookStore
                                  .getState()
                                  .emitBlockable('terminal.beforeExecute', {
                                    ...hookData,
                                    command: tool_params?.command || '',
                                  });
                                if (termResult.decision === 'deny') {
                                  get().updateToolCallResults({
                                    [tool.id]: {
                                      path: '',
                                      content: JSON.stringify({
                                        error: true,
                                        message: `Hook denied: ${termResult.reason || 'blocked by hook'}`,
                                      }),
                                      isError: true,
                                    },
                                  });
                        ...
                        } else if (tool.function.name === 'use_skill') {
                          otel.saveToolContext(
                            tool.id,
                            tool.function.arguments,
                            get().conversationRound,
                          );
                          handleUseSkillToolCall(tool.id, tool_params);

                          // task 工具已在 for 循环之前统一并行处理，这里只处理非 task 工具
                        } else {
                          if (
                            [
                              'edit_file',
                          ...
                          (async () => {
                            try {
                              ...
                              if (
                                ...
                                const fileResult = await useHookStore
                                  .getState()
                                  .emitBlockable('file.beforeEdit', {
                                    ...hookData,
                                    path:
                                      tool_params?.target_file ||
                                      tool_params?.path ||
                                      '',
                                  });
                                if (fileResult.decision === 'deny') {
                                  get().updateToolCallResults({
                                    [tool.id]: {
                                      ...
                                      content: JSON.stringify({
                                        error: true,
                                        message: `Hook denied: ${fileResult.reason || 'blocked by hook'}`,
                                      }),
                                      isError: true,
                                    },
                                  });
                ...
                set((state) => {
                  if (state.message) {
                        ...
                        if (now - throttleTime > 100) {
                          ...
                          for (const toolCall of toolCalls) {
                            if (
                              ...
                              for (
                                let i = toolcallParsedIndex;
                                i < argumentStr.length;
                                i++
                              ) {
                                parsedArgumentStr += argumentStr[i];
                                toolcallParsedIndex++;
                                if (
                                  parsedArgumentStr.endsWith('"target_file": "')
                                ) {
                                  inTargetFile = true;
                                } else if (inTargetFile) {
                                  parsedTargetFile += argumentStr[i];
                                  if (
                                    parsedArgumentStr.endsWith(
                                      '", "is_create_file"',
                                    )
                                  ) {
                                    inTargetFile = false;
                                    inIsCreateFile = true;
                                    parsedTargetFile = parsedTargetFile.slice(
                                      0,
                                      -19,
                                    );
                                  } else if (
                                    ...
                                    parsedArgumentStr.endsWith(', "diff": "')
                                  ) {
                                    inTargetFile = false;
                                    inDiff = true;
                                    parsedTargetFile = parsedTargetFile.slice(
                              ...
                              const result = parseWriteEditToolCall(
                                toolCall,
                                toolcallParsedIndex,
                              );
                              toolcallParsedIndex = result.nextParsedIndex;
                              if (result.codeContent) {
                                state.message.codeContent = result.codeContent;
                              }
                            }
                          }
                        }
                    ...
                    state.message.content = content;
                    state.message.reasoningContent =
              ...
              if (error.name !== ABORT_ERROR_NAME) {
                ...
                if (shouldRetry) {
                  ...
                  if (compressionSuccess) {
                    chatStoreState.updateCurrentSession((session) => {
                      if (session.data?.messages) {
                        const compressingMsgIndex =
                          session.data.messages.findIndex(
                            (msg) => msg.id === id + suffix,
                          );
            ...
            parentSpan: get().submitSpan?.span,
          },
        );
        userReporter.report({
      ...
      for (let i = 0; i < sendMessages.length; i++) {
        ...
        const isCludeThinking =
          chatModels[model]?.hasThinking &&
          getAIGWModel(model)?.toLocaleLowerCase?.()?.includes?.('claude');
      ...
      if (currentAttach?.type === ChatMessageAttachType.MultiAttachment) {
        ...
        messages[currentMessageIndex].content = await parseMentions(
          originContent,
          currentAttach.attachs,
        );
      }
      // 替换需要发送给模型的 prompt
      if (chatPromptStoreState.prompt) {
      ...
      if (
        [ChatModelSupplyChannel.DEEPSEEK, ChatModelSupplyChannel.QWEN].includes(
      ...
      if (
        [ChatModel.GPT5, ChatModel.GPT51, ChatModel.GPT51Codex].includes(model)
      ) {
        delete data.temperature;
      }
            ...
            set((state) => {
              if (state.message) {
                ...
                if (reasoningResponse?.reasoning_content) {
                  state.message.reasoning_content =
                    reasoningResponse.reasoning_content;
                }
              } else {
        ...
        chatStoreState.updateCurrentSession((session) => {
          ...
          if (unfinishedTaskCalls.length > 0) {
            ...
            unfinishedTaskCalls.forEach((toolCall) => {
              ...
              // 避免触发 updateToolCallResults → handleAutoExecute
              session.data?.messages.push({
                id: nanoid(),
                role: ChatRole.Tool,
              ...
              console.log(
                `[onStop] Added aborted tool message for task tool ${toolCall.id}`
              );
            });
          }
          ...
          Object.keys(chatApplyInfo).forEach((toolCallId) => {
            ...
            if (chatApplyItem.applying) {
              useChatApplyStore.getState().updateChatApplyItem(toolCallId, {
                applying: false,
              });
              const lastMessage =
                session.data?.messages[session.data?.messages.length - 1];
      ...
      set((state) => {
        ...
        if (!hasReplaced) {
          ...
          const dataWithSequence = {
            ...data,
            sequenceNumber:
              data.sequenceNumber || prePromptCodeBlock.length + 1,
            createdAt: Date.now(),
          };
    ...
    loadingMessage: '',
    startSubmitSpan: (isUserInput: boolean) => {
      const { submitSpan: oldSubmitSpan, conversationRound: oldRound } = get();
      ...
      const { chatType, codebaseChatMode } = chatStoreState;
      const scene: otel.TracingScene =
        chatType === 'default'
          ? 'default'
          : codebaseChatMode === 'openspec' || codebaseChatMode === 'speckit'
            ? 'spec_coding'
            : 'vibe';

      const { span, round } = otel.startSubmitSpan({
        isUserInput,
        topic,
        association: {
          userId: useAuthStore.getState().username || undefined,
          sessionId: chatStoreState.currentSessionId || undefined,
          scene,
          agentType: 'master_agent',
        },
        existingRound: isUserInput ? undefined : oldRound,
      });
      span.setAttributes({
        'gen_ai.request.model': model,
        'gen_ai.request.is_user_input': isUserInput,
      });

      set((state) => {
        state.submitSpan = span;
        state.conversationRound = round;
      });
    },
    endSubmitSpan: () => {
      const { submitSpan } = get();
      if (submitSpan) {
        submitSpan.end();
      }
      set((state) => {
        state.submitSpan = undefined;
      });
    },
    endConversationRound: () => {
      const { submitSpan, conversationRound } = get();
      if (submitSpan) {
        submitSpan.end();
      }
      if (conversationRound) {
        conversationRound.rootSpan.end();
      }
      otel.clearToolContextDataMap();
      set((state) => {
        state.submitSpan = undefined;
        state.conversationRound = undefined;
      });
    },
  })),
);
...
/**
 * 处理 use_skill 工具调用：支持单个或多个skills，内置 skill 直接返回内容，非内置走扩展流程
 */
function handleUseSkillToolCall(
  toolId: string,
  ...
  const builtinResults = skillNames.map((name: string) => ({
    name,
    content: getBuiltinSkillContent(name),
  }));

  const allBuiltin = builtinResults.every(r => r.content);

  if (allBuiltin) {
    // 所有都是内置skill：直接在 WebView 中返回结果(返回数组格式)
    const skillsData = builtinResults.map(r => ({
      name: r.name,
      content: r.content,
      path: 'builtin',
      source: 'builtin',
    ...
    window.postMessage(
      {
        ...
        data: {
          tool_name: 'use_skill',
          tool_result: {
            content: JSON.stringify(skillsData),
            isError: false,
          },
          tool_id: toolId,
        },
...
export type ChatAttachStore = {
  ...
  update: (attach?: ChatAttachStore['attachs']) => void;
};
...
export const useUserActionStore = create<ChatUserAction>((set, get) => ({
  ...
  resumeUserAction: async () => {
    const createdFiles = (getLocalStorage(EUserAction.CreateFile) ||
      {}) as Record<string, string[]>;
    const appliedCodeBlocks = (getLocalStorage(EUserAction.ApplyCodeBlock) ||
      {}) as Record<string, string[]>;
    set(() => ({
...
/**
 * 组装用户消息内容。
 * 返回 ChatMessageContentUnion[] 结构的内容数组，仅包含用户原始输入文本。
 *
 * Agent 任务约束指令现在通过独立的 system-reminder 消息注入，
 * 不再追加到用户消息末尾，以保持用户消息的纯净性。
 */
...
export function getContentString(content: string | ChatMessageContentUnion[]) {
  if (typeof content === 'string') {
    ...
    const currentContent = content
      ?.filter?.((i) => i?.type === ChatMessageContent.Text)
      ?.map?.((i) => i?.text)
      .join('\n');
...
function getFeatureLatestModified(feature: SpecKitFeatureInfo): number {
  ...
  if (feature.tasksFile?.lastModified)
    times.push(feature.tasksFile.lastModified);
  if (feature.researchFile?.lastModified)
    times.push(feature.researchFile.lastModified);
  if (feature.dataModelFile?.lastModified)
    times.push(feature.dataModelFile.lastModified);
  if (feature.quickstartFile?.lastModified)
    times.push(feature.quickstartFile.lastModified);
  if (feature.checklistFile?.lastModified)
    times.push(feature.checklistFile.lastModified);
...
export function autoSelectActiveChangeOrFeature(specInfo: SpecInfo): void {
  const chatStore = useChatStore.getState();
  const { chatType, codebaseChatMode, activeChangeId, activeFeatureId } =
    chatStore;
 ...
 * 抽取为独立函数以便复用（当前 session 处理和队列事件处理）
 */
function processSessionComplete(
  ...
  if (lastMessage.processing === false) {
    if (import.meta.env.DEV) {
      console.log(
        '[TaskEventBus] Skipping handleAutoExecute: already processed (processing=false)',
      );
    }
    return;
  }
  ...
  if (!allToolResultsComplete) {
    ...
    if (import.meta.env.DEV) {
      console.error(
        '[TaskEventBus] ⚠️ Tool results incomplete - missing results for:',
        {
          sessionId,
          missingToolCallIds,
          missingCount: missingToolCallIds.length,
          allToolCallIds: tool_calls.map(tc => ({ id: tc.id, name: tc.function.name })),
          existingResultIds,
          toolCallsCount: tool_calls.length,
          toolResultCount: existingResultIds.length,
        },
      );
    } else {
      // 生产环境也记录警告，但信息较少
  ...
  if (import.meta.env.DEV) {
    console.log(
      '[TaskEventBus] Triggering handleAutoExecute via event-driven path',
      { sessionId, hasActiveTaskTools },
    );
  }

  // ✅ 幂等检查增强：检查 isProcessing 状态
  // 如果 updateToolCallResults 已经触发了 handleAutoExecute（isProcessing=true），则跳过
  if (chatStreamStore.isProcessing) {
    if (import.meta.env.DEV) {
      console.log(
        '[TaskEventBus] Skipping handleAutoExecute: already in processing (isProcessing=true)',
      );
    }
    return;
  }
...
/**
 * 监听 SESSION_ALL_TASKS_COMPLETE 事件，触发 handleAutoExecute
 *
 * 这是事件驱动架构的**兜底路径**。正常情况下，updateToolCallResults 会在
 * 所有 tool_result 更新后立即触发 handleAutoExecute（主路径）。
 *
 * 但在某些异常情况下（如竞态条件、事件延迟），事件系统会作为备份机制，
 * 确保 handleAutoExecute 一定被触发。
 *
 * 幂等保证：
 * 1. 检查 lastMessage.processing 状态，避免重复执行
 * 2. 检查 chatStreamStore.isProcessing 状态，如果主路径已触发则跳过
 * 3. 事件持久化队列：非当前 session 的事件存入队列，切回时自动处理
 */
if (typeof window !== 'undefined') {
  taskEventBus.on('SESSION_ALL_TASKS_COMPLETE', (event) => {
    ...
    if (import.meta.env.DEV) {
      console.log(
        '[TaskEventBus] SESSION_ALL_TASKS_COMPLETE received:',
        { sessionId, taskCount: Object.keys(event.results).length },
      );
    }
    ...
    if (chatStore.currentSessionId !== sessionId) {
      // ========== 事件持久化队列：存储非当前 session 的事件 ==========
      pendingEventQueue.enqueue(sessionId, event);
      if (import.meta.env.DEV) {
        console.log(
          '[TaskEventBus] Event queued for session:',
          { eventSessionId: sessionId, currentSessionId: chatStore.currentSessionId },
        );
      }
      return;
    }
  ...
  useChatStore.subscribe((state) => {
    ...
    if (currentSessionId && currentSessionId !== prevSessionId) {
      ...
      if (pendingEvent) {
        if (import.meta.env.DEV) {
          console.log(
            '[TaskEventBus] Processing queued event for session:',
            currentSessionId,
          );
        }
  ...
  if (import.meta.env.DEV) {
    console.log('[TaskEventBus] SESSION_ALL_TASKS_COMPLETE listener registered with pending event queue support');
  }
}
