...
import type { Map as Y3Map } from '../env';
...
export class EditorObject<N extends Table.NameCN = Table.NameCN> {
    ...
    public get json(): Y3Json | undefined {
        if (this._json === undefined) {
            ...
            this._json = new Y3Json(this._text, fixedFloat, this.manager.openTable(this.tableName).template);
...
export class EditorTable<N extends Table.NameCN = Table.NameCN> extends vscode.Disposable {
    ...
    private _template: Record<string, any> | undefined;
    get template() {
     ...
     * @returns 
     */
    public async create(options?: CreateOptions<N>): Promise<EditorObject<N>>{
        ...
        if (options?.key) {
            ...
            if (!await this.canUseKey(key, options?.overwrite)) {
                throw new Error(l10n.t('key {0} 已存在', key));
            }
        } else {
        ...
        if (options?.copyFrom) {
            ...
            if (!template) {
                throw new Error(l10n.t('模板文件不存在'));
            }
...
export class EditorManager {
    constructor(public map: Y3Map, public rootUri: vscode.Uri) {}
