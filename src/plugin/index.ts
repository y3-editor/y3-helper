...
import type { Plugin as PluginType } from './plugin';
...
export async function runAllPlugins(map: Y3Map, funcName: string) {
...
export async function findPlugin(uri: vscode.Uri): Promise<PluginType | undefined> {
    if (!y3.env.project) {
        return undefined;
    }
    for (const map of y3.env.project.maps) {
