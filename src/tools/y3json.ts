...
export class Y3Json {
    ...
    constructor(text: string, public fixedFloat = false, public readonly template: Record<string, any> | null = null) {
        this._text = text;
    }
    ...
    private stringify() {
        ...
        function encode(value: any, currentData2?: any, template?: any, tabLevel = 0) {
            if (value === 'null') {
                return 'null';
            }
            switch (typeof value) {
                ...
                case 'object': {
                    ...
                    if (Array.isArray(value)) {
                        if (template?.['__tuple__'] === true && Array.isArray(template?.['items'])) {
                            return encode({
                                __tuple__: true,
                                items: value,
                            }, currentData2 ?? {
                        ...
                        for (let i = 0; i < value.length; i++) {
                            let item = value[i];
                            result += tab + encode(item, currentData2?.[i] ?? currentData2?.[0], template?.[0], tabLevel + 1);
                            if (i < value.length - 1) {
                                result += ', ';
                            }
                            result += '\n';
                        ...
                        for (let i = 0; i < keys.length; i++) {
                            let key = keys[i];
                            let item = value[key];
                            result += tab + encode(key) + ': ' + encode(item, currentData2?.[key], template?.[key], tabLevel + 1);
                            if (i < keys.length - 1) {
                                result += ', ';
                            }
                            result += '\n';
        ...
        let text = encode(data, data2, this.template);
